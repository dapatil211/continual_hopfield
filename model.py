...
class TinyEpisodicMemoryModel(BaseModel):
    def __init__(self, buffer_size, logit_masks, num_classes=100):
        super().__init__()
        self.buffer = ListBuffer(buffer_size)
        self.base = torchvision.models.resnet18(num_classes=100)
        self.task_logit_masks = logit_masks

    def get_loss(self, X, y, task_ids):
        batch_size = torch.shape
        er_X, er_y, er_task_ids = self.buffer.sample()
...
class GenerativeReplay(BaseModel):
    def __init__(self, logit_masks, image_size, image_channel_size,
                 critic_channel_size, generator_channel_size, num_classes=100):
        super().__init__()
        self.task_logit_masks = logit_masks

        self.base = torchvision.models.resnet18(num_classes=num_classes)
        self.gen = WGAN(image_size, image_channel_size, critic_channel_size, generator_channel_size)

        gen_g_optimizer = optim.Adam(
            self.gen.generator.parameters(),
            lr=1e-3, weight_decay=1e-5, betas=(.5, .9),
        )
        gen_c_optimizer = optim.Adam(
            self.gen.critic.parameters(),
            lr=1e-3, weight_decay=1e-5, betas=(.5, .9),
        )
        self.gen.set_lambda(10.)
